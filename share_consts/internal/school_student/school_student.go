		...
		IModules: co_module.NewModules[
			...
			*co_model.FdInvoiceDetailRes,
			...
			&co_model.Config{
				...
				KeyIndex:                       "School_student",
				RoutePrefix:                    "/student",
				StoragePath:                    "./resources/student",
				UserType:                       share_enum.User.Type.Student,
				Identifier: co_model.Identifier{
					Company:  "edu_school",
					Employee: "edu_school_student",
			...
			&co_dao.XDao{ // 以下为业务层实例化dao模型，如果不是使用默认模型时需要将自定义dao模型作为参数传进去，相同属性前缀需要配合使用不能拆开应用
				...
				Employee: co_dao.NewCompanyEmployee(share_dao.NewEduSchoolStudent()),
				//
